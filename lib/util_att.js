...
aofn.queryATTPARENT = function (key, extra) {
    var ans = {};
    ans[aofn.config.db.attach.fields.parentfld] = key;
    if (ans) {
...
aofn.queryATTATT = function (key, extra) {
    var ans = {};
    ans[aofn.config.db.attach.fields.attfld] = key;
    if (ans) {
        ans = aofn.mergeRecursive(ans, extra);
    }
    return ans;
...
/**
 *	queryATTX - Creates an object for a attached version
 */
aofn.queryATTX = function (fld, key, extra) {
    var ans = {};
    ans[fld] = key;
    if (ans) {
        ans = aofn.mergeRecursive(ans, extra);
 ...
 */
aofn.queryATTSTYLE = function (key, extra) {
    var ans = {};
    ans[aofn.config.db.attach.fields.stylefld] = key;
    if (ans) {
        ans = aofn.mergeRecursive(ans, extra);
    }
    return ans;
