...
app.get('/:db/:collection', function (req, res) {
    ...
    aofn.authorize(req, res, function (req, res) {
        ...
        if (query) {
            ...
            // This is an addition that allows for searching for each word in the string (aka eCandidus Quick Find)
            var md = aofn.getQValue(req, 'matchDESC');
            if (md) {
                var desc = aofn.regExpEscape(md).replace(/\w/g, '.+');
                var mdq = aofn.queryATT(aofn.config.db.fields.dec, new RegExp(desc));
                if (query) {
                    query = {
                        '$and': [query, mdq]
                    };
 ...
 */
app.get('/:db/:collection/' + aofn.config.metadata, function (req, res) {
    aofn.debugSTART(req);
 ...
 */
app.post('/:db/:collection/' + aofn.config.metadata, function (req, res) {
 ...
 *
 *      Deletes a collection metadata
 */
app.delete('/:db/:collection/' + aofn.config.metadata, function (req, res) {
    aofn.debugSTART(req, 'POST');
