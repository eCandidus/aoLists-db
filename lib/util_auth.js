...
aofn.customAUTH = function (req, res, username, password, endpoint) {
    ...
    //settings.level = 'user' // Set to aofn.config.db.users.mgrlevel for a manager
...
aofn.authorize = function (req, res, cbok, cbfail) {
    ...
    // Get passed values
    var username = aofn.userNRULE(authname || aofn.getQValue(req, 'u', 'username'));
    var password = aofn.userNRULE(authpwd || aofn.getQValue(req, 'p', 'password'), true);
        ...
        if (username) {
            ...
            if (aofn.response.workBUFFER(req).security) {
                ...
                if (aofn.response.workBUFFER(req).security.valid) {
                    ...
                    wb.mgrlevel = wb.level == aofn.config.db.users.mgrlevel;
                    // Call
                    if (cbok) {
                        cbok(req, res);
                    }
