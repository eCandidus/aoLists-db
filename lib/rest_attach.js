    ...
    app.get('/:db/:collection/:id/attached/:style?', function (req, res) {
        ...
        aofn.authorize(req, res, function (req, res) {
            ...
            if (aofn.config.enableDESC) {
                extra = aofn.queryATTX(aofn.config.db.attach.fields.desccfld, 1);
                descfld = aofn.config.db.attach.fields.desccfld;
            }
            options.fields = aofn.queryATTATT(1, aofn.queryATTSTYLE(1, aofn.queryATTX(aofn.config.db.attach.fields.vercfld, 1, extra)));
            if (req.params.style) {
                ...
                db.collection(req.params.collection2, function (err, collection) {
                    ...
                    collection.findOne(query, {}, function (err, doc) {
                        if (err) {
                            ...
                            if (doc) {
                                document = aofn.mergeRecursive(document, aofn.queryATTX(aofn.config.db.attach.fields.vercfld, doc[aofn.config.db.fields.ver] || ''));
                                if (aofn.config.enableDESC) {
                                    document = aofn.mergeRecursive(document, aofn.queryATTX(aofn.config.db.attach.fields.desccfld, doc[aofn.config.db.fields.desc] || ''));
                                }
                            }
                                ...
                                db.collection(req.params.collection, function (err, collection) {
                                    ...
                                    collection.findOne(query, {}, function (err, doc) {
                                        if (err) {
                                            ...
                                            if (doc) {
                                                document = aofn.mergeRecursive(document, aofn.queryATTX(aofn.config.db.attach.fields.verpfld, doc[aofn.config.db.fields.ver] || ''));
                                                if (aofn.config.enableDESC) {
                                                    document = aofn.mergeRecursive(document, aofn.queryATTX(aofn.config.db.attach.fields.descpfld, doc[aofn.config.db.fields.desc] || ''));
                                                }
                                            }
                                            aofn.executeDB(req, res, aofn.config.db.attach.db, function (req, res, db) {
                                                db.collection(aofn.config.db.attach.collection, function (err, collection) {
